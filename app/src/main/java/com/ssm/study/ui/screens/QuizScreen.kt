    ...
    Column(Modifier.fillMaxSize().padding(ScreenPadding), verticalArrangement = Arrangement.spacedBy(18.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            Column(Modifier.weight(1f)) {
                Text("${question.topic.displayName} · SSM ${question.year}", color = MaterialTheme.colorScheme.primary)
                Text("Question ${state.currentIndex + 1}/${state.questions.size}", style = MaterialTheme.typography.titleMedium)
        ...
        ElevatedCard(Modifier.fillMaxWidth()) {
            Text(question.stem, Modifier.padding(24.dp), style = MaterialTheme.typography.headlineSmall)
        }
